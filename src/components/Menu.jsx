    ...
    <section id="menu" className="bg-white">
      <div className="section-container">
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          {menuItems.map((item) => (
            <div key={item.id} className="flex flex-col sm:flex-row bg-cream/30 rounded-xl p-6 hover:shadow-lg transition-shadow border border-cream gap-6">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl text-charcoal leading-tight">{item.name}</h3>
                  <span className="text-terracotta font-bold ml-2">KES {item.price}</span>
                </div>
